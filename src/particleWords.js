...
class ParticleWords {
  ...
  getInitOptions() {
    ...
    var startX =
      centerX +
      Math.sqrt(
        startRadius * startRadius - (startY - centerY) * (startY - centerY)
      ) *
        direction;
    return {
  ...
  draw() {
    ...
    this.ctx.font =
      ...
      "px " +
      this.fontFamily +
      ", 'STKaiti', 'Lucida Handwriting', 'Arial'";
    var self = this;
    var statusCount = 0;
    ...
    this.winFrameId = window.requestAnimationFrame(function drawStep() {
      ...
      self.ctx.fillText(
        self.text,
        (self.cWidth - self.ctx.measureText(self.text).width) * 0.5,
        self.cHeight * 0.5
      );
  ...
  gatherParticle(imgData) {
    ...
    for (
      var i = 0;
      i < this.pixels.length && j < this.particles.length;
      i++, j++
    ) {
      try {
